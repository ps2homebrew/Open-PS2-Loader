...
void InstallSecrmanHook(void *exp)
{
	...
	if (old == NULL) old = DummySecrAuthCard;
...
static int CheckPatchMc2_s1() {
	...
	if (info.version < 0x102) goto not_found; // I'm sure mc2_s1 won't work with this old iop kernel

	if (getModInfo("mcman", &info)) goto not_found;  // mcman and mc2_s1 are mutually exclusive
	...
	if(!mod_table) goto not_found_again; // mc2_s1 not found
...
int hookRegisterLibraryEntires(iop_library_t *lib)
{
	...
	else if (!strncmp(lib->name, "mcman", 8))
	{
		...
		if (lib->version >= 0x208) InstallMcmanHook(&lib[1]);
	}
...
void hookSio2man(Sio2Packet *sd, Sio2McProc sio2proc)
{
	...
	if (memcards[ctrl & 0x1].mcnum != -1)
	{
		...
		if (ctrl == 0x81) sio2proc = Sio2McEmu;
	}
	...
	if(skip_sema_wait) {
		if((SIO2STAT & 0xf000) != 0x1000) { // uh oh
			...
			if(ra[0]) {
				...
				else DPRINTF("hookSio2man67 failed to find stat6c check, mc access may fail\n");
				FlushDcache();
				FlushIcache();
			}
		}
			...
			if (wdma[0] == 0x81)
			{
				...
				if (mcd->mcnum != -1)
				{
					...
					switch (wdma[1])
					{
						...
						case 0x42:
							SioResponse(mcd, rdma, length);    
							if (result) result = MceWrite(mcd, &wdma[3], wdma[2]);     
							else DPRINTF("skipping write command after I/O error.\n");
							break;
							/* 0x81 0x43 - Read data from a memory card */
						case 0x43:
							length = wdma[2]; /* length = (*pctl >> 18) & 0x1FF; */
							SioResponse(mcd, rdma, 5);
							if (result) result = MceRead(mcd, &rdma[4], length);        
							else DPRINTF("skipping read command after I/O error.\n");      
							if (!result) mips_memset(&rdma[4], 0xFF, length);
