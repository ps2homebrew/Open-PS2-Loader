...
void menuDrawStatic() {
	...
	if (gTheme->menuText.enabled) {
		...
		fntRenderString(gTheme->menuText.posX, gTheme->menuText.posY, gTheme->menuText.aligned, GetMenuItemText(selected_item->item), gTheme->menuText.color);
	}
	...
	if (cur) {
		...
		while (ps && (others < gTheme->displayedItems)) {
			...
			fntRenderString(itemsList->posX + stretchedSize, curpos + others * MENU_ITEM_HEIGHT, ALIGN_NONE, submenuItemGetText(&ps->item), color);
		...
		if (gTheme->itemText.enabled) {
			if (selected_item->item->userdata && (cur->item.id != -1)) {
				item_list_t *support = selected_item->item->userdata;
				fntRenderString(gTheme->itemText.posX, gTheme->itemText.posY, gTheme->itemText.aligned, support->itemGetStartup(cur->item.id), gTheme->itemText.color);
			}
		}
	}
