...
int bt_probe(int devId)
{
    ...
    if ((config->bNumInterfaces < 1) || (config->wTotalLength < (sizeof(UsbConfigDescriptor) + sizeof(UsbInterfaceDescriptor)))) {
        DPRINTF("BT: Error - No interfaces available\n");
        return 0;
    }

    intf = (UsbInterfaceDescriptor *)((char *)config + config->bLength);

    DPRINTF("BT: bInterfaceClass %X bInterfaceSubClass %X bInterfaceProtocol %X\n",
...
int bt_connect(int devId)
{
    ...
    if (bt_dev.eventEndp < 0 || bt_dev.inEndp < 0 || bt_dev.outEndp < 0) {
        bt_release();
        DPRINTF("BT: Error - connect failed: not enough endpoints! \n");
        return -1;
    }
    ...
    if ((bt_dev.hci_sema = CreateSema(&SemaData)) < 0) {
        DPRINTF("BT: Failed to allocate I/O semaphore.\n");
        return -1;
    }
...
static uint8_t hci_read_bdaddr();
    ...
    if (!result) {
        ...
        switch (hci_buf[0]) { // switch on event type
            case HCI_EVENT_COMMAND_COMPLETE:
                ...
                if ((hci_buf[3] == 0x09) && (hci_buf[4] == 0x10)) {
                    for (i = 0; i < 6; i++)
                        dg_bdaddr[i] = hci_buf[6 + i];

                    hci_event_flag_ |= HCI_FLAG_READ_BDADDR;
                }
...
static uint8_t chrg_inited = 0;

void ds3bt_init_charging()
{
    if (!chrg_inited)
...
void *rpc_sf(int cmd, void *data, int size)
{
    switch (cmd) {
        case DS3BT_INIT:
            ds3bt_init(*(uint8_t *)data);
            break;
        case DS3BT_INIT_CHARGING:
            ds3bt_init_charging();
            break;
        case DS3BT_GET_STATUS:
        ...
        case DS3BT_SET_RUMBLE:
            ...
            break;
        case DS3BT_SET_LED:
            ds3bt_set_led(*(uint8_t *)(data + 1), *(uint8_t *)data);
            break;
        case DS3BT_GET_DATA:
            ds3bt_get_data((uint8_t *)data, 18, *(uint8_t *)data);
            break;
        case DS3BT_RESET:
            ds3bt_reset();
            break;
